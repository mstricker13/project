...
def run(gt_path, expert_path, value_start_index, max_horizon, horizon_index, window_s, freq, out_path):
    ...
    with tqdm(total=data_length) as pbar:
        ...
        for y in range(data_length):
            ...
            branch_0 = Conv1D(32, 3, strides=1, padding='same', activation='relu', kernel_initializer=glorot_uniform(1))(
                inputs_n)
            branch_1 = branch_0
            ...
            net = Dense(horizon, name='dense_final', activity_regularizer=regularizers.l2(0.05))(
                branch_0)  # activity regularizer value based on data
            net = add([net, inputs_k])

            model = Model(inputs=[inputs_n, inputs_k], outputs=net)
            opt = Adam(lr=0.0001)
            callback = ModelCheckpoint(filepath=os.path.join(out_path, str(y) + 't.h5'), monitor='val_loss', save_best_only=True,
                                       save_weights_only=True)

            model.compile(loss='mean_squared_error', optimizer=opt)

            model.fit({'input_n': train_input, 'input_k': k_train}, y_train, validation_data=[[val_input, k_val], y_val],
                      callbacks=[callback], shuffle=True, batch_size=8, epochs=100, verbose=0)

            model.load_weights(os.path.join(out_path, str(y) + 't.h5'))
            ...
            pbar.update(1)
        np.savetxt(os.path.join(out_path, 'final_25_trend_horg.csv'), final_predictions, fmt='%1.3f', delimiter=',')