...
def sMAPE(prediction, ground_truth, horizon, transformer, fileprefix, series_counter):
    ...
    for pred, gt in zip(prediction, ground_truth):
        ...
        #reversed_prediction = reversed_prediction[0]
        #reversed_prediction = math.exp(reversed_prediction[0])
        #for rescaler
        #reversed_prediction = transformer.inverse_transform([pred])
        #reversed_prediction = reversed_prediction[0][0]
        #reversed_prediction = math.exp(reversed_prediction)
        #
        #only log
        #reversed_prediction = math.exp(pred[0])
        #nothing
...
def sMAPE_paper(prediction, ground_truth, horizon, norm_val, fileprefix, series_counter, seasonality_list):
    prediction = prediction[0]
    ground_truth = ground_truth[0]
    add = 0
    #TODO somehow check if first iteration and delete old file
    ...
    for pred, gt, season in zip(prediction, ground_truth, seasonality_list):
        ...
        #reversed_prediction = reversed_prediction + reversed_season
...
def sMAPE_self_stand(prediction, ground_truth, horizon, mean, std, fileprefix, series_counter):
    #print('----------------------------------------')
    #print(horizon)
    #print(prediction)
    #print(ground_truth)
    ...
    for pred, gt in zip(prediction, ground_truth):
        ...
        #reversed_prediction = math.exp(pred[0])
        #nothing
        reversed_prediction = ((pred[0] * std) + mean)
